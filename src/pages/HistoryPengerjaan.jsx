...
const HistoryPengerjaan = () => {
  const location = useLocation();
  const token = Cookies.get('authToken');
  const [user, setUser] = useState(null);
  const [skorKuis, setSkorKuis] = useState(0);
  const { historyData, skor } = location.state || {};
    ...
    <div className='bg-slate-100 min-h-screen'>
      <div className="fixed left-0 right-0 top-0 bg-white z-10 md:px-8 border-b shadow-sm">
        <div className="max-w-[1300px] px-8 mx-auto flex justify-between items-center py-4">
          ...
          <button
            className='px-4 py-1 border rounded bg-red-500 text-white text-sm font-medium'
            onClick={() => navigate('/')}
          >
            Kembali
          </button>
      ...
      <div className='max-w-[1000px] px-8 mx-auto pt-24'>
        <div className='grid grid-cols-2 mb-5 gap-2 items-center  text-center rounded-lg h-[100px] text-white'>
          <h2 className='col-span-2 text-4xl font-semibold  border bg-violet-500 py-5 rounded-lg flex flex-col'><span className='text-xl'>{user?.username}</span> </h2>
...
export default HistoryPengerjaan;
